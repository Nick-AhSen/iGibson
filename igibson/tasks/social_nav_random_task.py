...
class SocialNavRandomTask(PointNavRandomTask):
    ...
    def load_static_obstacle(self, env):
        logging.info("Loading static obstacle")
        # WALL A
        # p1 = env.scene.map_to_world(np.array([224, 99]))
        # p2 = env.scene.map_to_world(np.array([224, 115]))
        # p3 = env.scene.map_to_world(np.array([28, 115]))
        # p4 = env.scene.map_to_world(np.array([28, 99]))
        # print("p1 = ",p1)
        # print("p2 = ",p2)
        # print("p3 = ",p3)
        # print("p4 = ",p4)
        # #initial_bbox = np.array(p1, p2, p3, p4)
        ...
        # WALL F
        p1 = env.scene.map_to_world(np.array([27, 310]))
        p2 = env.scene.map_to_world(np.array([27, 333]))
        p3 = env.scene.map_to_world(np.array([113, 333]))
        p4 = env.scene.map_to_world(np.array([113, 422]))
        p5 = env.scene.map_to_world(np.array([136, 422]))
        p6 = env.scene.map_to_world(np.array([136, 333]))
        p7 = env.scene.map_to_world(np.array([223, 333]))
        p8 = env.scene.map_to_world(np.array([223, 310]))
        p9 = env.scene.map_to_world(np.array([136, 310]))
        p10 = env.scene.map_to_world(np.array([136, 226]))
        p11 = env.scene.map_to_world(np.array([113, 226]))
        p12 = env.scene.map_to_world(np.array([113, 310]))

        n1 = env.scene.map_to_world(np.array([27, 422]))
        n2 = env.scene.map_to_world(np.array([223, 422]))
        n3 = env.scene.map_to_world(np.array([223, 226]))
        n4 = env.scene.map_to_world(np.array([27, 226]))

        self.orca_sim.addObstacle([tuple(p3), tuple(p2), tuple(n1),tuple(p4)])
        self.orca_sim.addObstacle([tuple(p7), tuple(p6), tuple(p5), tuple(n2)])
        self.orca_sim.addObstacle([tuple(p8), tuple(n3), tuple(p10), tuple(p9)])
        self.orca_sim.addObstacle([tuple(p12), tuple(n4), tuple(p11), tuple(p1)])
        
        # Corners extracted for WALL F
        # corners is  [314.5583    26.558304]#
        # corners is  [331.4417    26.558304]#
        # corners is  [225.5583   116.558304]#
        # corners is  [314.18127 116.18128]
        # corners is  [331.5583   116.441696]#
        # corners is  [422.4417   116.558304]#
        # corners is  [225.5583 133.4417]#
        # corners is  [422.4417 133.4417]#
        # corners is  [314.4417 133.5583]#
        # corners is  [331.5583 133.5583]#
        # corners is  [314.5583 223.4417]#
        # corners is  [331.4417 223.4417]#
        
        self.orca_sim.processObstacles()

    # Only for interactive scenes
    ...
    def sample_ped_point(self, env):
        # sample based on map
        trav = env.scene.ped_trav_map
    ...
    def sample_initial_pos(self, env, ped_id):
        ...
        while must_resample_pos:
            initial_pos = self.sample_ped_point(env)
    ...
    def sample_new_target_pos(self, env, initial_pos, ped_id=None):
        ...
        while True:
            if self.offline_eval:
                ...
                # Don't sample goal point that's too close to the start point.
                # TODO: I think there might be a parameter for the distance 
                while np.linalg.norm(target_pos-initial_pos) < 3:
                    target_pos = self.sample_ped_point(env)
    ...
    def get_task_obs(self, env):
        ...
        """
        # print("target pos is {}".format(self.target_pos))
        #task_obs = (self.target_pos[:2])
        ...
        ##task_obs.append([linear_velocity, angular_velocity])
        #task_obs.append([env.scene.floor_map[0]])
        #TODO pre process this map to occupancy grid
        #task_obs = np.append(task_obs, )

        source = env.robots[0].get_position()[:2]
        target = self.target_pos[:2]
        waypoints = env.scene.get_shortest_path(0, source, target, entire_path=True)
    
        task_obs = [self.target_pos[:2], [linear_velocity, angular_velocity], waypoints]
